...
public class WorldMap implements Serializable {
	...
	/** 
	 * build the name of this location based on its generated position on the map graphic image.
	 * 
	 ...
	 * 
	 * 
	 * @param curRank the current rank of this city(not used)
	 * @param LocationNode node
...
//		double maxRankDouble = (double) MAX_RANK;
		...
		//set the territory for this node and all its outbound trails
		node.setTerritory(rankIndex);
		for (TrailEdge trail : node.getOutboundTrails()){
			trail.setTerritory(rankIndex);			
		}
...
// - replaced with call in location node	convertToMapCoords(this.mapNodes.get(finalDestination.getRank()).get(0));
	...
	/**
	 * this function will set to visible all nodes and trails within a particular territory.
	 * it is intended to be used by the purchased/found map item
	 * 
	 * @param ConstantStore.StateIdx the StateIdx enum that we wish to set all the node(locations and trails) values to be visible in 
	 */
		
	public void setVisibleByArea(ConstantStore.StateIdx visibleState){
		for (int rank = 0; rank < this.MAX_RANK; rank++){
			for (LocationNode node : this.mapNodes.get(rank)){
				//if this node is in the territory we are setting to be visible, set it and its out-trails to be visible
