...
export class TownScene extends Scene {
    ...
    init() {
        ...
        const ground = new ParallaxComponentLoop(TownScene.CANVAS_WIDTH, ImageStore.getImage('GRASS'), 0.5);
        parallaxPanel.addParallaxComponent(ground);
        ground.setPosition({ x: 0, y: this.groundY }, ReferencePoint.TOPLEFT);
        const trail = new ParallaxComponentLoop(TownScene.CANVAS_WIDTH, ImageStore.getImage('TRAIL'), 0.5);
        parallaxPanel.addParallaxComponent(trail);
        trail.setPosition({ x: 0, y: this.groundY + 80 }, ReferencePoint.TOPLEFT);
        const hillB = new ParallaxComponentLoop(TownScene.CANVAS_WIDTH, ImageStore.getImage('HILL_B'), 0.7);
        parallaxPanel.addParallaxComponent(hillB);
        hillB.setPosition({ x: 0, y: this.groundY - 120 }, ReferencePoint.TOPLEFT);
        const hillA = new ParallaxComponentLoop(TownScene.CANVAS_WIDTH, ImageStore.getImage('HILL_A'), 0.8);
        ...
        if (this.location.getTrails() > 0) {
            ...
            this.trailChoiceModal = new ComponentModal(TownScene.CANVAS_WIDTH, TownScene.CANVAS_HEIGHT, this, getLiteral('TOWN_SCENE', 'TRAIL_CHOICE'), buttonCount, segControl);
            if (hasTracking) {
