...
import { getItemTypeName, getItemTypeCost, getItemTypeWeight } from '../model/ItemType';
export class StoreScene extends Scene {
    constructor(party, storeInventory, priceModifier) {
        super();
        this.buttonMap = [];
        this.storeCounters = [];
    ...
    init() {
        const backgroundPanel = new Panel(StoreScene.CANVAS_WIDTH, StoreScene.CANVAS_HEIGHT, StoreScene.BACKGROUND_COLOR);
    ...
    createComponents() {
        ...
        this.partyMoneyLabel = new Label(StoreScene.INVENTORY_BUTTON_WIDTH * 4 + StoreScene.PADDING * 3, StoreScene.REGULAR_BUTTON_HEIGHT, fieldFont, Color.white, '');
        ...
        for (let i = 0; i < populatedSlots.length && i < 16; i++) {
            ...
            const counter = new Counter(StoreScene.INVENTORY_BUTTON_WIDTH, StoreScene.INVENTORY_BUTTON_HEIGHT, itemLabel);
        ...
        this.cancelButton = new Button(200, StoreScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(fieldFont, Color.white, getLiteral('GENERAL', 'LEAVE') || 'Leave'));
        this.cancelButton.addClickListener(() => this.onCancelClick());
        this.cancelButton.layout();
        this.clearButton = new Button(120, StoreScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(fieldFont, Color.white, 'Clear'));
        this.clearButton.addClickListener(() => this.onClearClick());
        this.clearButton.setDisabled(true);
        this.clearButton.layout();
        this.inventoryButton = new Button(120, StoreScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(fieldFont, Color.white, 'Inventory'));
        this.inventoryButton.addClickListener(() => this.onInventoryClick());
        this.inventoryButton.layout();
        this.buyButton = new Button(120, StoreScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(fieldFont, Color.white, getLiteral('GENERAL', 'CONFIRM') || 'Buy'));
        this.buyButton.addClickListener(() => this.onBuyClick());
        this.buyButton.setDisabled(true);
        this.buyButton.layout();
    }
    layoutComponents() {
        const storePanel = new Panel(StoreScene.INVENTORY_BUTTON_WIDTH * 4 + StoreScene.PADDING * 5, StoreScene.INVENTORY_BUTTON_HEIGHT * 4 + StoreScene.PADDING * 5);
        this.mainLayer.add(storePanel);
        storePanel.setPosition({ x: 0, y: 0 }, ReferencePoint.TOPLEFT, StoreScene.PADDING, StoreScene.PADDING);
        storePanel.addAsGrid(this.storeCounters, storePanel.getPosition(ReferencePoint.TOPLEFT), 4, 4, StoreScene.PADDING, StoreScene.PADDING, StoreScene.PADDING, StoreScene.PADDING);
        this.mainLayer.add(this.partyMoneyLabel);
        this.partyMoneyLabel.setPosition(storePanel.getPosition(ReferencePoint.BOTTOMCENTER), ReferencePoint.TOPCENTER, 0, StoreScene.PADDING);
        this.mainLayer.add(this.textPanel);
        this.textPanel.setPosition({ x: StoreScene.CANVAS_WIDTH, y: 0 }, ReferencePoint.TOPRIGHT, -StoreScene.PADDING, StoreScene.PADDING);
        this.textPanel.addAsColumn([
            this.itemNameLabel,
            this.itemDescLabel,
            ...
            this.totalCostLabel
        ], this.textPanel.getPosition(ReferencePoint.TOPLEFT), StoreScene.PADDING, StoreScene.PADDING, StoreScene.PADDING);
        this.mainLayer.add(this.clearButton);
        this.clearButton.setPosition({ x: StoreScene.CANVAS_WIDTH, y: StoreScene.CANVAS_HEIGHT }, ReferencePoint.BOTTOMRIGHT, -StoreScene.PADDING, -StoreScene.PADDING);
        this.mainLayer.add(this.buyButton);
        this.buyButton.setPosition(this.clearButton.getPosition(ReferencePoint.TOPLEFT), ReferencePoint.TOPRIGHT, -StoreScene.PADDING, 0);
        this.mainLayer.add(this.cancelButton);
        ...
        this.mainLayer.add(this.inventoryButton);
        this.inventoryButton.setPosition({
            x: storePanel.getPosition(ReferencePoint.BOTTOMRIGHT).x,
            y: this.buyButton.getPosition(ReferencePoint.TOPLEFT).y
        }, ReferencePoint.TOPRIGHT, 0, 0);
    }
    getButtonIndex(itemType) {
...
StoreScene.PADDING = 20;
StoreScene.REGULAR_BUTTON_HEIGHT = 30;
StoreScene.INVENTORY_BUTTON_WIDTH = 130;
StoreScene.INVENTORY_BUTTON_HEIGHT = 90;
StoreScene.BACKGROUND_COLOR = Color.fromHex(0x0C5DA5);
StoreScene.TEXT_PANEL_COLOR = Color.fromHex(0x679FD2);
StoreScene.CANVAS_WIDTH = 1024;
StoreScene.CANVAS_HEIGHT = 576;
