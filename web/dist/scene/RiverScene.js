...
export class RiverScene extends Scene {
    constructor(party, canvasWidth, canvasHeight) {
        super();
        this.nextModal = null;
        this.crossTime = 0;
        this.haveWaited = false;
        this.waiting = false;
        this.crossingBridge = false;
        this.crossingRiver = false;
        this.didTakeDamage = false;
        this.bobbingUp = true;
        this.party = party;
    ...
    init() {
        ...
        this.bridge.setPosition({ x: this.canvasWidth / 2, y: this.canvasHeight / 2 }, ReferencePoint.CENTERCENTER, 100, 100);
        ...
        if (!this.isPaused()) {
            ...
            if (this.waiting) {
                ...
                if (this.crossingRiver) {
                    // Bobbing animation
                    if (this.bobbingUp) {
                        this.wagon.setPosition({ x: this.wagon.getX(), y: this.wagon.getY() - 1 }, ReferencePoint.TOPLEFT);
                    }
...
RiverScene.NUM_CLOUDS = 5;
RiverScene.CLOUD_OFFSET = 20;
RiverScene.CLOUD_DISTANCE_VARIANCE = 10;
RiverScene.CLOUD_OFFSET_VARIANCE = 10;
RiverScene.CLOUD_DISTANCE = 300;
RiverScene.MAX_RIVER_DEPTH = 8;
RiverScene.FORD_DANGER_DEPTH = 3;
RiverScene.CAULK_DANGER_DEPTH = 4;
RiverScene.RIVER_CROSS_TIME = 3000;
RiverScene.BRIDGE_CROSS_TIME = 5500;
//# sourceMappingURL=RiverScene.js.map