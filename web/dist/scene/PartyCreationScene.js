...
export class PartyCreationScene extends Scene {
    constructor(canvasWidth, canvasHeight) {
        super();
        this.peopleData = [];
        this.newPersonButtons = [];
        this.personNameTextFields = [];
        this.personGenderControls = [];
        this.personProfessionPanels = [];
        this.personChangeProfessionButtons = [];
        this.personProfessionLabels = [];
        this.personMoneyLabels = [];
        this.personSkillPanels = [];
        this.personChangeSkillButtons = [];
        this.personSkillLabels = [];
    ...
    createProfessionControl() {
        ...
        this.professionSegmentedControl = new SegmentedControl(800, 200, 5, 5, 5, true, 1, ...professionLabels);
        this.professionSegmentedControl.setTooltips(professionTooltips);
    }
    createSkillControl() {
        const skills = Object.values(Skill).filter(s => s !== Skill.NONE);
        const skillLabels = skills.map(s => getSkillName(s));
        this.skillSegmentedControl = new SegmentedControl(800, 200, 5, 3, 5, true, PartyCreationScene.NUM_SKILLS, ...skillLabels);
    }
    createPersonColumn(index, font) {
        const prevButton = index === 0 ? null : this.newPersonButtons[index - 1];
        const newPersonButton = new Button(this.buttonWidth, PartyCreationScene.NEW_PERSON_BUTTON_HEIGHT, Label.withTextWidth(font, Color.white, 'New Person'));
        newPersonButton.addClickListener(() => this.onNewPerson(index));
        newPersonButton.layout();
        if (prevButton) {
            newPersonButton.setPosition(prevButton.getPosition(ReferencePoint.TOPRIGHT), ReferencePoint.TOPLEFT, PartyCreationScene.PADDING, 0);
        }
        else {
        ...
        nameField.setPosition(newPersonButton.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT, 0, PartyCreationScene.INNER_PADDING);
        this.mainLayer.add(nameField);
        this.personNameTextFields[index] = nameField;
        const genderControl = new SegmentedControl(this.buttonWidth, PartyCreationScene.REGULAR_BUTTON_HEIGHT, 1, 2, 0, false, 1, 'Male', 'Female');
        genderControl.setVisible(false);
        genderControl.setPosition(nameField.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT, 0, PartyCreationScene.INNER_PADDING);
        ...
        const changeProfessionButton = new Button(this.buttonWidth, PartyCreationScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(font, Color.white, 'Change Profession'));
        ...
        professionPanel.add(changeProfessionButton, professionPanel.getPosition(ReferencePoint.TOPLEFT), ReferencePoint.TOPLEFT);
        this.personChangeProfessionButtons[index] = changeProfessionButton;
        const professionLabel = new Label(this.buttonWidth, PartyCreationScene.REGULAR_BUTTON_HEIGHT, font, Color.white, 'No Profession');
        professionLabel.setBackgroundColor(Color.darkGray);
        professionLabel.setAlignment(Alignment.CENTER);
        professionPanel.add(professionLabel, changeProfessionButton.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT);
        this.personProfessionLabels[index] = professionLabel;
        professionPanel.setPosition(genderControl.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT, 0, PartyCreationScene.INNER_PADDING);
        ...
        moneyLabel.setPosition(professionLabel.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT, 0, PartyCreationScene.INNER_PADDING);
        ...
        const changeSkillButton = new Button(this.buttonWidth, PartyCreationScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(font, Color.white, 'Change Skills'));
        ...
        skillPanel.add(changeSkillButton, skillPanel.getPosition(ReferencePoint.TOPLEFT), ReferencePoint.TOPLEFT);
        ...
        for (let j = 0; j < PartyCreationScene.NUM_SKILLS; j++) {
            const skillLabel = new Label(this.buttonWidth, PartyCreationScene.REGULAR_BUTTON_HEIGHT, font, Color.white, 'Empty');
            ...
            skillPanel.add(skillLabel, prevLabel.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT);
            this.personSkillLabels[index][j] = skillLabel;
        }
        skillPanel.setPosition(moneyLabel.getPosition(ReferencePoint.BOTTOMLEFT), ReferencePoint.TOPLEFT, 0, PartyCreationScene.INNER_PADDING);
        this.mainLayer.add(skillPanel);
        this.personSkillPanels[index] = skillPanel;
    }
    createBottomControls(font) {
        this.rationsSegmentedControl = new SegmentedControl(200, PartyCreationScene.REGULAR_BUTTON_HEIGHT, 1, 3, 0, true, 1, 'Barebones', 'Meager', 'Filling');
        this.rationsSegmentedControl.setSelection([1]);
        this.rationsSegmentedControl.setPosition({ x: PartyCreationScene.PADDING, y: this.canvasHeight - PartyCreationScene.PADDING - PartyCreationScene.REGULAR_BUTTON_HEIGHT }, ReferencePoint.TOPLEFT);
        this.mainLayer.add(this.rationsSegmentedControl);
        this.paceSegmentedControl = new SegmentedControl(200, PartyCreationScene.REGULAR_BUTTON_HEIGHT, 1, 3, 0, true, 1, 'Steady', 'Strenuous', 'Grueling');
        this.paceSegmentedControl.setSelection([0]);
        this.paceSegmentedControl.setPosition(this.rationsSegmentedControl.getPosition(ReferencePoint.TOPRIGHT), ReferencePoint.TOPLEFT, PartyCreationScene.PADDING, 0);
        this.mainLayer.add(this.paceSegmentedControl);
        this.confirmButton = new Button(150, PartyCreationScene.REGULAR_BUTTON_HEIGHT, Label.withTextWidth(font, Color.white, 'Confirm'));
        this.confirmButton.addClickListener(() => this.onConfirm());
        this.confirmButton.layout();
        this.confirmButton.setPosition({ x: this.canvasWidth - PartyCreationScene.PADDING, y: this.canvasHeight - PartyCreationScene.PADDING - PartyCreationScene.REGULAR_BUTTON_HEIGHT }, ReferencePoint.TOPRIGHT);
    ...
    onChangeProfession(index) {
        this.currentPersonModifying = index;
        const modal = new ComponentModal(this.canvasWidth, this.canvasHeight, this, 'Select Profession', 2, this.professionSegmentedControl);
        this.showModal(modal);
    }
    onChangeSkills(index) {
        this.currentPersonModifying = index;
        ...
        const modal = new ComponentModal(this.canvasWidth, this.canvasHeight, this, 'Select Skills', 2, this.skillSegmentedControl);
        this.showModal(modal);
    }
...
PartyCreationScene.PADDING = 20;
PartyCreationScene.INNER_PADDING = 10;
PartyCreationScene.NUM_PEOPLE = 4;
PartyCreationScene.NUM_SKILLS = 3;
PartyCreationScene.NEW_PERSON_BUTTON_HEIGHT = 100;
PartyCreationScene.REGULAR_BUTTON_HEIGHT = 30;
//# sourceMappingURL=PartyCreationScene.js.map