...
export class HuntHUD extends HUD {
  ...
  private static readonly BUTTON_HEIGHT = HUD.HEIGHT - 2 * HuntHUD.MARGIN;

  private ammoLabel: Label;
  private campButton: Button;
  private inventoryButton: Button;

  constructor(width: number, onCamp: () => void, onInventory: () => void) {
    ...
    this.campButton = new Button(
      100,
      HuntHUD.BUTTON_HEIGHT,
      Label.withTextWidth(fieldFont, Color.white, 'Camp')
    );
  ...
  setAmmo(bullets: number, boxes: number): void {
