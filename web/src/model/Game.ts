...
export class Game {
  ...
  resetStoreInventory(location: LocationNode): void {
    ...
    for (let i = 0; i < startItems.length; i++) {
      if (
        startItems[i] === ItemType.STRANGEMEAT ||
...
function isItemTypeFood(itemType: ItemType): boolean {
  return (
