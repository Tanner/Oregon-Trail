...
export class OptionsScene extends Scene {
  ...
  private volumeCounter!: Counter;
  private saveModal!: ComponentModal<SegmentedControl>;

  // Callback for requesting scene transitions
  ...
  private init(): void {
    ...
    const volumeButtonLabel = new Label(
      OptionsScene.BUTTON_WIDTH,
      fieldFont.getLineHeight(),
      fieldFont,
      Color.white,
    ...
    this.volumeCounter = new Counter(
      OptionsScene.BUTTON_WIDTH,
      OptionsScene.BUTTON_HEIGHT,
      volumeButtonLabel
    );
  ...
  private handleMainMenuButton(): void {
    if (this.onMainMenu) {
      this.onMainMenu();
    }
  }
   ...
   */
  static loadGame(slot: number): any | null {
    const key = `oregon-trail-save-${slot}`;
