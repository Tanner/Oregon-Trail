...
const itemEncounterData: Record<
  ItemEncounterType,
  { itemType: ItemType; message: string }
> = {
  ...
  [ItemEncounterType.BREAD]: {
    ...
    message:
      "You find a basket of bread which contains %d %s %s. You decide to take it.",
  },
  [ItemEncounterType.GUN]: {
    itemType: ItemType.GUN,
    message:
      "You find %d rusted %s on the trail. It seems like it still works.",
  },
...
function getItemEncounterMessage(
  type: ItemEncounterType,
  number: number,
): string {
  ...
  if (
    getItemTypePluralName(data.itemType) === getItemTypeName(data.itemType)
  ) {
    if (number === 1) {
      itemPrefix = "piece of";
    }
...
export class ItemEncounter extends Encounter {
  ...
  private type: ItemEncounterType;
  ...
  constructor(party: Party, value: number) {
    ...
    this.type = types[index] as ItemEncounterType;
    this.numItems = 0;
  }
