...
export class PartyInventoryScene extends Scene {
  ...
  private binItems: Map<ItemType, number> = new Map();

  private closeButton: Button;
  private transferButton: Button;
  private dropButton: Button;
  private binCounter: Counter;

  private inventoryPanels: Panel[] = [];
  ...
  constructor(canvasWidth: number, canvasHeight: number, party: Party) {
    ...
    this.closeButton = new Button(
      150,
      PartyInventoryScene.BUTTON_HEIGHT,
      Label.withTextWidth(fieldFont, Color.white, 'Close')
    );
    ...
    this.transferButton = new Button(
      150,
      PartyInventoryScene.BUTTON_HEIGHT,
      Label.withTextWidth(fieldFont, Color.white, 'Transfer')
    );
    ...
    this.dropButton = new Button(
      150,
      PartyInventoryScene.BUTTON_HEIGHT,
      Label.withTextWidth(fieldFont, Color.white, 'Drop')
    );
  ...
  private layoutInventory(canvasWidth: number, canvasHeight: number): void {
    const fieldFont = FontStore.getFont(FontID.FIELD);
  ...
  private createPersonPanel(person: Person, width: number): Panel {
  ...
  private createVehiclePanel(vehicle: any, width: number): Panel {
    const panel = new Panel(width, 100);
    const fieldFont = FontStore.getFont(FontID.FIELD);
