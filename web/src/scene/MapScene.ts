...
export class MapScene extends Scene {
  ...
  private currNode: LocationNode;
  private currTrail: TrailEdge;
  private currLocPtr: AnimatingSprite;
  private currLocParty: AnimatingSprite;
  private locationButtons: (Button | null)[] = [];
  private returnToCamp: Button;
  ...
  private initialize(): void {
    ...
    this.returnToCamp = new Button(
      returnCampWidth,
      MapScene.REGULAR_BUTTON_HEIGHT,
      Label.withTextWidth(
        fieldFont,
        Color.white,
        constantGet('MAP_SCENE', 'RETURN_CAMP')
      )
    );
    ...
    this.returnToCamp.setPosition(
      { x: 20, y: containerHeight - 20 },
      ReferencePoint.BOTTOMLEFT
    );

    const mapBackground = new Panel(
      containerWidth,
      containerHeight,
      ImageStore.getImage('TRAIL_MAP')
    );
    this.backgroundLayer.add(mapBackground);
  }

  private setupAnimatedPointer(): void {
    const pointerFrames: HTMLImageElement[] = [];
    for (let i = 6; i >= 1; i--) {
      pointerFrames.push(ImageStore.getImage(`MAP_POINTER${i}`));
    }
    this.currLocPtr.addAnimation('default', pointerFrames, 100);

    const partyFrames: HTMLImageElement[] = [];
    for (let i = 1; i <= 4; i++) {
  ...
  private setupLegend(fieldFont: any, containerWidth: number): void {
    const legend1 = new Label(
      MapScene.PADDING * 6,
      fieldFont.getCharHeight(),
      fieldFont,
      Color.white,
      'Legend'
    );
    ...
    legend1.setPosition(
      { x: containerWidth - 2 * legend1.getWidth(), y: legend1.getWidth() / 4 },
      ReferencePoint.TOPRIGHT
    );

    const legendItems: Array<{ size: number; color: Color; text: string }> = [
    ...
    let previousButton: Button | null = null;
    ...
    for (const item of legendItems) {
      ...
      if (previousButton === null) {
        button.setPosition(
          { x: legend1.getX() - MapScene.PADDING / 2, y: referenceY },
          ReferencePoint.TOPRIGHT
        );
      } else {
        button.setPosition(
          { x: previousButton.getX() + previousButton.getWidth(), y: previousButton.getY() + previousButton.getHeight() + MapScene.PADDING / 2 },
          ReferencePoint.TOPRIGHT
        );
      }
      ...
      label.setPosition(
        { x: button.getX() + button.getWidth() + MapScene.PADDING, y: button.getY() + button.getHeight() / 2 },
  ...
  private getLocColor(node: LocationNode): Color {
